...
class TreeNode:
    ...
    def __init__(self, value):
        ...
        self.height: int = 1 #new node should have height of 0 instead of 3
...
class AVLTree:
    ...
    def _traverse_from_node(self, local_root: TreeNode) -> list:
        ...
        if local_root is not None: #don't traverse if no root means if local_root is not None
    ...
    def _insert_at(self, root: TreeNode, new) -> TreeNode:
        ...
        if root is None:
            ...
            # Add to the left
            root.left = self._insert_at(root.left, new) #should insert at left instead of right
        else:
            # Add to the right
        ...
        if balance < -1:
            if self._get_balance(root.right) > 0:
                ...
                return self._left_rotate(root) #missing return for Case 3
            else:
    ...
    def _left_rotate(self, local_root: TreeNode) -> TreeNode:
        ...
        # Update heights
        local_root.height = 1 + self._get_max_height_of_children(local_root) #height should increase by 1 and not 2
        right_child.height = 1 + self._get_max_height_of_children(right_child) #height should increase by 1 and not 2
    ...
    def _right_rotate(self, local_root: TreeNode) -> TreeNode:
        ...
        local_root.left = left_right_grandchild #left_right_grandchild value should be assigned to local_root.left instead of local_root.right for a right rotation
    ...
    def _get_max_height_of_children(self, local_root: TreeNode) -> int:
        ...
        return max(left_height, right_height) #to get max height of children, it should use max function instead of min function
